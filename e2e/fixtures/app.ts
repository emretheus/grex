...
// Shared Playwright fixture that boots the Codewit shell past its first-run
// gates so specs start from the main workspace surface:
//   - stubs `get_app_settings` to mark onboarding completed so the overlay
//     never appears
//   - exposes `window.__CODEWIT_E2E__` as a hook for specs to override
...
type CodewitFixtures = {
	app: Page;
};

export const test = base.extend<CodewitFixtures>({
	app: async ({ page }, use) => {
		await page.addInitScript(() => {
			window.__CODEWIT_E2E__ = {
				invokeOverrides: {
