...
const MODEL_SLUG_SET_BY_PROVIDER: Record<ProviderKind, ReadonlySet<ModelSlug>> = {
  ...
  qwenCode: new Set(MODEL_OPTIONS_BY_PROVIDER.qwenCode.map((option) => option.slug)),
  auggie: new Set(MODEL_OPTIONS_BY_PROVIDER.auggie.map((option) => option.slug)),
  goose: new Set(MODEL_OPTIONS_BY_PROVIDER.goose.map((option) => option.slug)),
};
...
export function getGeminiThinkingModelAlias(
  ...
  if (kind === "level" && nextOptions.thinkingLevel) {
    return `codewit-gemini-${base}-thinking-level-${nextOptions.thinkingLevel.toLowerCase()}`;
  }
  if (kind === "budget" && nextOptions.thinkingBudget !== undefined) {
    ...
    return `codewit-gemini-${base}-thinking-budget-${budget}`;
  }
  return null;
}
