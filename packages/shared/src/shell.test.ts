...
describe("extractPathFromShellOutput", () => {
  it("extracts the path between capture markers", () => {
    expect(
      extractPathFromShellOutput(
        "__CODEWIT_PATH_START__\n/opt/homebrew/bin:/usr/bin\n__CODEWIT_PATH_END__\n",
      ),
...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__CODEWIT_ENV_PATH_START__\n/a:/b\n__CODEWIT_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__CODEWIT_ENV_PATH_START__");
    expect(args?.[1]).toContain("__CODEWIT_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__CODEWIT_ENV_PATH_END__",
        "__CODEWIT_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__CODEWIT_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      [
        "__CODEWIT_ENV_CUSTOM_VAR_START__",
        "  padded value  ",
        "__CODEWIT_ENV_CUSTOM_VAR_END__",
      ].join("\n"),
    );
