# `/grex-cli stack` — plan & build a stacked PR
...
In Grex a stack is a **chain of workspaces**: one workspace = one branch = one PR, linked by `parentWorkspaceId`. The sidebar groups a stack's workspaces together (tip on top, base at the bottom); `gh pr create --base` automatically targets the parent branch.
...
- `grex workspace list --json` — scan open workspaces for related branches or an in-flight stack (rows in the same repo, or already linked by a parent).
- `grex workspace stack <ref>` — if a candidate is already stacked, see its whole chain.
- `grex workspace show <ref>` — inspect a candidate's branch, target, and PR.
...
- **From a stack that already exists** (workspaces are created): pipe the live data straight through —
  ```bash
  grex workspace stack <ref> --json | python3 scripts/render_stack.py -
  ```
...
- **Each higher layer** (forks off the layer below; its PR targets the parent): `grex workspace new --parent <lower-workspace-ref>` — the first higher layer's `<lower-workspace-ref>` is your current workspace.
...
### 5. Ship bottom-up
Ship and merge the **bottom** PR first. After a lower layer changes or merges, re-sync the layers above it with `/grex-cli restack` (see `references/restack.md`).
