...
5. A successful recap is saved under `codewit:thread-recaps:v1` with its covered message id, source signature, and update time. The cache is sanitized on read and pruned to the freshest 80 threads so localStorage stays bounded.
