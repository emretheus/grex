...
class Logger {
	...
	constructor() {
		const envLevel = process.env.CODEWIT_LOG?.toLowerCase();
		const level: Level =
		...
		// shells inheriting CODEWIT_LOG_DIR from a release codewit would otherwise
		// pollute ~/codewit/logs. Bun auto-sets NODE_ENV=test.
		const isTest =
