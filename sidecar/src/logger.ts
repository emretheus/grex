...
class Logger {
	...
	constructor() {
		const envLevel = process.env.GREX_LOG?.toLowerCase();
		const level: Level =
		...
		// shells inheriting GREX_LOG_DIR from a release grex would otherwise
		// pollute ~/grex/logs. Bun auto-sets NODE_ENV=test.
		const isTest =
