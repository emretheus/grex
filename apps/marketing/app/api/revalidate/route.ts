 ...
 * `published` events, so grex.ai reflects new versions within seconds
 * instead of waiting for the hourly `revalidate: 3600` window in
 * `app/page.tsx` + `lib/github.ts`.
 *
 * Security:
 *   - Shared secret `GREX_MARKETING_REVALIDATE_SECRET` must match on both
...
export async function POST(req: Request): Promise<NextResponse> {
	const expected = process.env.GREX_MARKETING_REVALIDATE_SECRET;
	if (!expected) {
