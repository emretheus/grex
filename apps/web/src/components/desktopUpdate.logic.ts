...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  const action = resolveDesktopUpdateButtonAction(state);
  if (action === "download") {
    return "This Mac has Apple Silicon, but Codewit is still running the Intel build under Rosetta. Codewit is preparing the native Apple Silicon update.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but Codewit is still running the Intel build under Rosetta. Click Update to restart into the native Apple Silicon build.";
  }
  return "This Mac has Apple Silicon, but Codewit is still running the Intel build under Rosetta. The next app update will replace it with the native Apple Silicon build.";
}
