...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.codewit/worktrees/codewit-mvp/codewit-4e609bb8",
    );
    expect(result).toBe("codewit-4e609bb8");
  });

  it("keeps legacy .t3 worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.t3/worktrees/codewit-mvp/codewit-legacy123",
    );
    expect(result).toBe("codewit-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.codewit\\worktrees\\codewit-mvp\\codewit-4e609bb8",
    );
    expect(result).toBe("codewit-4e609bb8");
  });

  it("uses the final segment even when outside ~/.codewit/worktrees", () => {
