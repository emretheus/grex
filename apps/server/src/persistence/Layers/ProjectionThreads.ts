    ...
    execute: (row) =>
      sql`
        ...
        VALUES (
          ...
          ${!row.linkedIssues || !Array.isArray(row.linkedIssues) || row.linkedIssues.length === 0 ? null : JSON.stringify(row.linkedIssues)},
          ${row.latestTurnId},
          ${row.handoff === null ? null : JSON.stringify(row.handoff)},
          ${row.pinnedMessages === null ? null : JSON.stringify(row.pinnedMessages)},
          ${!row.threadMarkers || !Array.isArray(row.threadMarkers) || row.threadMarkers.length === 0 ? null : JSON.stringify(row.threadMarkers)},
          ${row.notes},
    ...
    execute: ({ threadId }) =>
      sql`
        SELECT
          ...
          linked_issue_json AS "linkedIssues",
