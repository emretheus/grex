...
const AUTO_DETACHED_WORKTREE_DIRNAME = "codewit";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `codewit-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const tempIndexDir = yield* fileSystem.makeTempDirectoryScoped({
            prefix: `codewit-git-status-index-${process.pid}-`,
          });
    ...
    const stashAndCheckout: GitCoreShape["stashAndCheckout"] = (input) =>
      Effect.gen(function* () {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["stash", "push", "-u", "-m", `codewit: stash before switching to ${input.branch}`],
          {
            timeoutMs: 30_000,
