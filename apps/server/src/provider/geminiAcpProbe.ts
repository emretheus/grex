...
function formatGeminiDiscoveryWarning(detail: string): string {
  return `Gemini CLI is installed, but Codewit could not verify authentication or discover models. ${detail}`;
}
...
function formatGeminiModelDiscoveryFallbackMessage(): string {
  return "Gemini CLI is installed and authenticated, but it did not report any available models. Codewit will use its built-in Gemini model list.";
}
