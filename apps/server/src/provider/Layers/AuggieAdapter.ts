/**
 * AuggieAdapterLive — Auggie CLI (`auggie --acp`) via ACP.
 *
...
import { makeAuggieAcpRuntime, type AuggieAcpRuntimeSettings } from "../acp/AuggieAcpSupport.ts";
import { AuggieAdapter, type AuggieAdapterShape } from "../Services/AuggieAdapter.ts";
import { type EventNdjsonLogger, makeEventNdjsonLogger } from "./EventNdjsonLogger.ts";

const PROVIDER = "auggie" as const;
...
function clearAuggieActiveTurn(ctx: AuggieSessionContext, turnId: TurnId): boolean {
...
function resolveAuggieAssistantItemTurnId(
...
function completeAuggieAssistantItemTurnId(
  ctx: AuggieSessionContext,
  itemId: string,
): TurnId | undefined {
  const turnId = resolveAuggieAssistantItemTurnId(ctx, itemId);
...
function resolveAuggieSessionCwd(
...
export function makeAuggieAdapter(
  auggieSettings: AuggieAcpRuntimeSettings,
  ...
  return Effect.gen(function* () {
    ...
    const startSession: AuggieAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const cwd = resolveAuggieSessionCwd(input.cwd, serverConfig);
          if (cwd === undefined) {
            return yield* new ProviderAdapterValidationError({
              provider: PROVIDER,
              operation: "startSession",
          ...
          const providerAuggieOptions = input.providerOptions?.auggie;
          const effectiveAuggieSettings: AuggieAcpRuntimeSettings = {
            ...(auggieSettings.binaryPath !== undefined
              ? { binaryPath: auggieSettings.binaryPath }
              : {}),
            ...(providerAuggieOptions?.binaryPath !== undefined
              ? { binaryPath: providerAuggieOptions.binaryPath }
              : {}),
          };

          const acp = yield* makeAuggieAcpRuntime({
              ...
              Effect.gen(function* () {
                switch (event._tag) {
                  ...
                  case "AssistantItemStarted": {
                    const turnId = resolveAuggieAssistantItemTurnId(ctx, event.itemId);
                    yield* offerRuntimeEvent(
                      makeAcpAssistantItemEvent({
                        stamp: yield* makeEventStamp(),
                        provider: PROVIDER,
                  ...
                  case "AssistantItemCompleted": {
                    const turnId = completeAuggieAssistantItemTurnId(ctx, event.itemId);
                    yield* offerRuntimeEvent(
                      makeAcpAssistantItemEvent({
                        stamp: yield* makeEventStamp(),
                        provider: PROVIDER,
                  ...
                  case "ContentDelta": {
                    ...
                    yield* offerRuntimeEvent(
                      makeAcpContentDeltaEvent({
                        ...
                        provider: PROVIDER,
                        threadId: ctx.threadId,
                        turnId: resolveAuggieAssistantItemTurnId(ctx, event.itemId),
      ...
      Effect.gen(function* () {
        ...
        const runPrompt = ctx.acp.prompt({ prompt: promptParts }).pipe(
          ...
          Effect.matchEffect({
            onFailure: (error) =>
              Effect.gen(function* () {
                if (!clearAuggieActiveTurn(ctx, turnId)) {
                  return;
                }
    ...
    const respondToUserInput: AuggieAdapterShape["respondToUserInput"] = (
      threadId,
      requestId,
      answers,
    ) =>
    ...
    const getComposerCapabilities: NonNullable<
      AuggieAdapterShape["getComposerCapabilities"]
    > = () =>
      Effect.succeed({
        provider: PROVIDER,
    ...
    const listModels: NonNullable<AuggieAdapterShape["listModels"]> = () =>
      Effect.succeed({
        models: [],
        source: "auggie.static",
        cached: true,
...
export const AuggieAdapterLive = Layer.effect(AuggieAdapter, makeAuggieAdapter({}));

export function makeAuggieAdapterLive(
  auggieSettings: AuggieAcpRuntimeSettings = {},
  options?: AuggieAdapterLiveOptions,
) {
  return Layer.effect(AuggieAdapter, makeAuggieAdapter(auggieSettings, options));
}
