/**
 * QwenCodeAdapterLive — Qwen Code CLI (`qwen --acp`) via ACP.
 *
...
import {
  makeQwenCodeAcpRuntime,
  type QwenCodeAcpRuntimeSettings,
} from "../acp/QwenCodeAcpSupport.ts";
import { QwenCodeAdapter, type QwenCodeAdapterShape } from "../Services/QwenCodeAdapter.ts";
import { type EventNdjsonLogger, makeEventNdjsonLogger } from "./EventNdjsonLogger.ts";

const PROVIDER = "qwenCode" as const;
const QWEN_CODE_RESUME_VERSION = 1 as const;
...
function clearQwenCodeActiveTurn(ctx: QwenCodeSessionContext, turnId: TurnId): boolean {
...
function resolveQwenCodeAssistantItemTurnId(
  ctx: QwenCodeSessionContext,
...
function completeQwenCodeAssistantItemTurnId(
  ctx: QwenCodeSessionContext,
  itemId: string,
): TurnId | undefined {
  const turnId = resolveQwenCodeAssistantItemTurnId(ctx, itemId);
...
function parseQwenCodeResume(raw: unknown): { sessionId: string } | undefined {
  if (!isRecord(raw)) return undefined;
  if (raw.schemaVersion !== QWEN_CODE_RESUME_VERSION) return undefined;
...
function resolveQwenCodeSessionCwd(
...
export function makeQwenCodeAdapter(
  qwenCodeSettings: QwenCodeAcpRuntimeSettings,
  ...
  return Effect.gen(function* () {
    ...
    const requireSession = (
      threadId: ThreadId,
    ): Effect.Effect<QwenCodeSessionContext, ProviderAdapterSessionNotFoundError> => {
    ...
    const stopSessionInternal = (ctx: QwenCodeSessionContext) =>
      Effect.gen(function* () {
    ...
    const startSession: QwenCodeAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const cwd = resolveQwenCodeSessionCwd(input.cwd, serverConfig);
          if (cwd === undefined) {
            return yield* new ProviderAdapterValidationError({
              provider: PROVIDER,
              operation: "startSession",
          ...
          let ctx!: QwenCodeSessionContext;

          const resumeSessionId = parseQwenCodeResume(input.resumeCursor)?.sessionId;
          const acpNativeLoggers = makeAcpNativeLoggers({
            nativeEventLogger,
            provider: PROVIDER,
            threadId: input.threadId,
          });
          const providerQwenCodeOptions = input.providerOptions?.qwenCode;
          const effectiveQwenCodeSettings: QwenCodeAcpRuntimeSettings = {
            ...(qwenCodeSettings.binaryPath !== undefined
              ? { binaryPath: qwenCodeSettings.binaryPath }
              : {}),
            ...(providerQwenCodeOptions?.binaryPath !== undefined
              ? { binaryPath: providerQwenCodeOptions.binaryPath }
              : {}),
          };

          const acp = yield* makeQwenCodeAcpRuntime({
            qwenCodeSettings: effectiveQwenCodeSettings,
            childProcessSpawner,
            cwd,
          ...
          const session: ProviderSession = {
            ...
            threadId: input.threadId,
            resumeCursor: {
              schemaVersion: QWEN_CODE_RESUME_VERSION,
              ...
              Effect.gen(function* () {
                switch (event._tag) {
                  ...
                  case "AssistantItemStarted": {
                    const turnId = resolveQwenCodeAssistantItemTurnId(ctx, event.itemId);
                    yield* offerRuntimeEvent(
                      makeAcpAssistantItemEvent({
                        stamp: yield* makeEventStamp(),
                        provider: PROVIDER,
                  ...
                  case "AssistantItemCompleted": {
                    const turnId = completeQwenCodeAssistantItemTurnId(ctx, event.itemId);
                    yield* offerRuntimeEvent(
                      makeAcpAssistantItemEvent({
                        stamp: yield* makeEventStamp(),
                        provider: PROVIDER,
                  ...
                  case "ContentDelta": {
                    ...
                    yield* offerRuntimeEvent(
                      makeAcpContentDeltaEvent({
                        ...
                        provider: PROVIDER,
                        threadId: ctx.threadId,
                        turnId: resolveQwenCodeAssistantItemTurnId(ctx, event.itemId),
          ...
          yield* offerRuntimeEvent({
            ...
            provider: PROVIDER,
            threadId: input.threadId,
            payload: { state: "ready", reason: "Qwen Code ACP session ready" },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
      ...
      Effect.gen(function* () {
        ...
        const runPrompt = ctx.acp.prompt({ prompt: promptParts }).pipe(
          ...
          Effect.matchEffect({
            onFailure: (error) =>
              Effect.gen(function* () {
                if (!clearQwenCodeActiveTurn(ctx, turnId)) {
                  return;
                }
    ...
    const interruptTurn: QwenCodeAdapterShape["interruptTurn"] = (threadId) =>
      Effect.gen(function* () {
    ...
    const getComposerCapabilities: NonNullable<
      QwenCodeAdapterShape["getComposerCapabilities"]
    > = () =>
      Effect.succeed({
        provider: PROVIDER,
    ...
    const listModels: NonNullable<QwenCodeAdapterShape["listModels"]> = () =>
      Effect.succeed({
        models: [],
        source: "qwen-code.static",
        cached: true,
...
export const QwenCodeAdapterLive = Layer.effect(QwenCodeAdapter, makeQwenCodeAdapter({}));

export function makeQwenCodeAdapterLive(
  qwenCodeSettings: QwenCodeAcpRuntimeSettings = {},
  options?: QwenCodeAdapterLiveOptions,
) {
  return Layer.effect(QwenCodeAdapter, makeQwenCodeAdapter(qwenCodeSettings, options));
}
