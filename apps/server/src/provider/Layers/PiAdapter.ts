      ...
      Effect.gen(function* () {
        ...
        if (loadedExtensions.length > 0) {
          ...
          offerRuntimeEvent({
            ...
            payload: {
              message:
                "Pi extensions are loaded, but Codewit does not yet support Pi extension UI APIs. Non-UI extension behavior should work, but extensions that call ctx.ui.* for prompts, widgets, confirmations, or status updates may not behave correctly.",
              detail: {
    ...
    const respondUnsupported = (threadId: ThreadId, method: string) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method,
          detail: `Pi does not expose Codewit approval/user-input requests for thread ${threadId}.`,
        }),
      );
