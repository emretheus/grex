...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  const previousDisableFlag = process.env.CODEWIT_DISABLE_CODEX_BROWSER_PLUGIN;

  afterEach(() => {
    if (previousCodewitHome === undefined) delete process.env.CODEWIT_HOME;
    else process.env.CODEWIT_HOME = previousCodewitHome;
    if (previousDisableFlag === undefined) delete process.env.CODEWIT_DISABLE_CODEX_BROWSER_PLUGIN;
    else process.env.CODEWIT_DISABLE_CODEX_BROWSER_PLUGIN = previousDisableFlag;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.CODEWIT_HOME = "/codewit-test/runtime";
    delete process.env.CODEWIT_DISABLE_CODEX_BROWSER_PLUGIN;
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/codewit-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });

  it("returns the source generated_images directory when the codewit-browser plugin is enabled", () => {
    process.env.CODEWIT_HOME = "/codewit-test/runtime";
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.codewit/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.codewit/runtime/codex-home-overlay";
