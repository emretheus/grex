...
The only Grex-operated service in the [mobile browser companion](../../docs/mobile-browser-companion-plan.md)
architecture. A Cloudflare Worker that writes **one `remote-<random>.grex.ai`
...
- It only **creates/deletes CNAMEs in the `remote-*` namespace**. Even though the
  API token is zone-wide (`grex.ai`), the Worker refuses to operate on any
  hostname outside `^remote-…\.grex\.ai$`, and on revoke it deletes only the
...
| `CF_API_TOKEN` | **Worker secret** | `Zone:DNS:Edit` on `grex.ai`; never committed |
| `CF_ZONE_ID` | `wrangler.jsonc` var | the `grex.ai` zone id |
| `ROOT_DOMAIN` | `wrangler.jsonc` var | `grex.ai` |
