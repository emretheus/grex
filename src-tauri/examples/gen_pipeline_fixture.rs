...
//!
//! The DB path resolves the same way the main app does: `~/grex` for
//! release builds, `~/grex-dev` for debug builds, or `GREX_DATA_DIR`
//! when set.
...
fn load_session_records(session_id: &str) -> Result<Vec<HistoricalRecord>> {
    let db_path = grex_lib::data_dir::db_path()?;
    let conn =
    ...
    let rows = stmt.query_map([session_id], |row| {
        ...
        let role: grex_lib::pipeline::types::MessageRole = row.get(1)?;
