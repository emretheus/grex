...
//!
//! The DB path resolves the same way the main app does: `~/codewit` for
//! release builds, `~/codewit-dev` for debug builds, or `CODEWIT_DATA_DIR`
//! when set.
...
fn load_session_records(session_id: &str) -> Result<Vec<HistoricalRecord>> {
    let db_path = codewit_lib::data_dir::db_path()?;
    let conn =
    ...
    let rows = stmt.query_map([session_id], |row| {
        ...
        let role: codewit_lib::pipeline::types::MessageRole = row.get(1)?;
