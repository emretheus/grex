...
/// test at a time touches the `GREX_DATA_DIR` env var.
...
impl TestEnv {
    pub fn new(name: &str) -> Self {
        ...
        let root = env::temp_dir().join(format!("grex-test-{name}-{}", Uuid::new_v4()));
        env::set_var("GREX_DATA_DIR", root.display().to_string());
...
impl GitTestRepo {
    ...
    pub fn init() -> Self {
        ...
        Self::run_git(p, &["config", "user.email", "test@grex.test"]);
