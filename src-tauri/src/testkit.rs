...
/// test at a time touches the `CODEWIT_DATA_DIR` env var.
...
impl TestEnv {
    pub fn new(name: &str) -> Self {
        ...
        let root = env::temp_dir().join(format!("codewit-test-{name}-{}", Uuid::new_v4()));
        env::set_var("CODEWIT_DATA_DIR", root.display().to_string());
...
impl Drop for TestEnv {
    fn drop(&mut self) {
        env::remove_var("CODEWIT_DATA_DIR");
...
impl GitTestRepo {
    ...
    pub fn init() -> Self {
        ...
        Self::run_git(p, &["config", "user.email", "test@codewit.test"]);
