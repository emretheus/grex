...
interface StagePackageJson {
  ...
  readonly codewitCommitHash: string;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "CODEWIT_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("CODEWIT_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "CODEWIT_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("CODEWIT_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("CODEWIT_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("CODEWIT_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("CODEWIT_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("CODEWIT_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("CODEWIT_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("CODEWIT_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("CODEWIT_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(
    Config.option,
  ),
});
...
function resolveGitHubPublishConfig():
  ...
  | undefined {
  const rawRepo =
    process.env.CODEWIT_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
const verifyStagedNodePty = Effect.fn("verifyStagedNodePty")(function* (
  ...
  yield* runCommand(
    ChildProcess.make({
      ...
      env: {
        ...process.env,
        CODEWIT_NODE_PTY_SMOKE_REQUIRE_ROOT: stageAppDir,
      },
      ...commandOutputOptions(verbose),
...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: "com.t3tools.codewit",
    productName,
    artifactName: "Codewit-${version}-${arch}.${ext}",
    directories: {
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `codewit-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    name: "codewit-desktop",
    version: appVersion,
    buildVersion: appVersion,
    codewitCommitHash: commitHash,
    private: true,
    description: "Codewit desktop build",
    author: "Emanuele Di Pietro",
    main: "apps/desktop/dist-electron/main.js",
    build: yield* createBuildConfig(
      options.platform,
      options.target,
      desktopPackageJson.productName ?? "Codewit",
      options.signed,
      options.mockUpdates,
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: CODEWIT_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: CODEWIT_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription(
      "Build arch, for example arm64/x64/universal (env: CODEWIT_DESKTOP_ARCH).",
    ),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: CODEWIT_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: CODEWIT_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: CODEWIT_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: CODEWIT_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: CODEWIT_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: CODEWIT_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: CODEWIT_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.string("mock-update-server-port").pipe(
    Flag.withDescription("Mock update server port (env: CODEWIT_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Codewit."),
