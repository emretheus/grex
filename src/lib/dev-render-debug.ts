...
declare global {
	interface Window {
		__CODEWIT_DEV_RENDER_STATS__?: DevRenderStats;
	}
}
...
function ensureStats(): DevRenderStats | null {
	...
	if (!window.__CODEWIT_DEV_RENDER_STATS__) {
		window.__CODEWIT_DEV_RENDER_STATS__ = {
			composer: {
				rendersByContext: {},
	...
	return window.__CODEWIT_DEV_RENDER_STATS__;
}
